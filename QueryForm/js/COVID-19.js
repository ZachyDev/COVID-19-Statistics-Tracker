    ...
    button.addEventListener('click',(event)=>{
        ...
        document.write(`<h1 style="color:gold;font-family:verdana">${country} COVID-19 Statistics</h1>`)
        // this is the ajax fetch request to COVID-19 api         
            fetch(`https://covid-19-data.p.rapidapi.com/country?format=json&name=${country}`, {
                ...
                .then((data)=>{
                    // statistics
                    let confirmedCases = `Confirmed Cases: ${data[0].confirmed} `;
                    let recoveredCases = `Recovered Cases: ${data[0].recovered} `;
                    let deathCases = `Death Cases: ${data[0].deaths} `;
